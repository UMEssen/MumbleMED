            ...
            try:
                ...
                for segment in tts_segments:
                    ...
                    audio_path = (
                        ...
                        / f"{document_id}_sentence_{sentence_id}_{speaker_id}.wav"
                    )
...
def _compute_split_stats(df_split: pd.DataFrame) -> dict:
    ...
    if "was_rechunked_after_tts_transform" in df_split.columns:
        stats["rechunked_after_tts_transform"] = int(
            df_split["was_rechunked_after_tts_transform"].sum()
        )
...
def validate_real_config(config: RealDatasetConfig) -> None:
    ...
    if not config.use_default_voice:
        voices_path = pathlib.Path(
            os.getenv("SPEAKER_VOICES_PATH")
            or (pathlib.Path(__file__).resolve().parents[2] / "speaker-voices")
        )
